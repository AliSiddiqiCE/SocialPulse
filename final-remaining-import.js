...
async function main() {
  console.log('Final remaining Next Retail import\n');
  
  let totalNew = 0;
  
  // Complete remaining Instagram Hashtag (867/1157 remaining: 290)
  console.log('Completing Instagram Hashtag (290 remaining)...');
  ...
  for (let i = 868; i < igHashtagLines.length && igHashtagImported < 290; i++) {
    ...
    if (fields.length >= 11) {
      try {
        ...
        `, fields.slice(0, 11));
        igHashtagImported++;
        if (igHashtagImported % 50 === 0) {
          console.log(`  Instagram Hashtag: ${igHashtagImported}/290 imported`);
        }
      } catch (error) {
        // Skip invalid
      }
  ...
  // Import YouTube Official with correct structure - 598 records
