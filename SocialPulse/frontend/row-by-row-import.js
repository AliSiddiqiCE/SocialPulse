...
async function completeRemainingImports() {
  ...
  // Complete Instagram Hashtag (33/1157 remaining: 1124)
  console.log('Completing Instagram Hashtag (1124 remaining)...');
  ...
  for (let i = 34; i < igHashtagLines.length && igHashtagImported < 1124; i++) {
    ...
    if (fields.length >= 11) {
      try {
        ...
        if (igHashtagImported % 100 === 0) {
          console.log(`  Instagram Hashtag: ${igHashtagImported}/1124 imported`);
        }
      } catch (error) {
        // Skip invalid
      }
...
async function main() {
  try {
    await completeRemainingImports();
  } catch (error) {
