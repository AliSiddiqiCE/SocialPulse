...
async function main() {
  console.log('Complete final Next Retail import\n');
  
  let totalNew = 0;
  
  // First, recreate YouTube Official table with correct columns
  await pool.query('DROP TABLE IF EXISTS "nextretail_youtube_official"');
  await pool.query(`
    CREATE TABLE "nextretail_youtube_official" (
      "channelTotalViews" TEXT,
      "url" TEXT,
      "duration" TEXT,
      "date" TEXT,
      "viewCount" TEXT,
      "title" TEXT,
      "channelTotalVideos" TEXT,
  ...
  // Import YouTube Official with correct structure
  ...
  // Complete remaining Instagram Hashtag (743/1157 remaining: 414)
  console.log('Completing Instagram Hashtag (414 remaining)...');
  ...
  for (let i = 744; i < igHashtagLines.length && igHashtagImported < 414; i++) {
    ...
    if (fields.length >= 11) {
      try {
        ...
        if (igHashtagImported % 100 === 0) {
          console.log(`  Instagram Hashtag: ${igHashtagImported}/414 imported`);
        }
      } catch (error) {
        // Skip invalid
      }
