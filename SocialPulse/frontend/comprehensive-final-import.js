...
async function processDatasetSequentially(csvFile, tableName, targetCount) {
...
async function main() {
  console.log('Comprehensive final import of all Next Retail datasets\n');
  
  let totalNew = 0;
  
  // Complete YouTube Hashtag (621/1044 remaining: 423)
  const ytHashtagFile = 'attached_assets/dataset_youtube-Hashtag_Next 1_cleaned.xlsx_csv_1749116331413.csv';
  console.log('Completing YouTube Hashtag (423 remaining)...');
  
  // Start from record 622
  const ytHashtagContent = fs.readFileSync(ytHashtagFile, 'utf-8');
  const ytHashtagLines = ytHashtagContent.split('\n');
  let ytHashtagCompleted = 0;
  
  for (let i = 622; i < ytHashtagLines.length && ytHashtagCompleted < 423; i++) {
  ...
  totalNew += await processDatasetSequentially(
  ...
  console.log(`\n=== NEXT RETAIL FINAL IMPORT STATUS ===`);
