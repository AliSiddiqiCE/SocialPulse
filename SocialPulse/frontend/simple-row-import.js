...
async function main() {
  console.log('Simple row import of remaining Next Retail datasets\n');
  
  let totalNew = 0;
  
  // Complete Instagram Hashtag (289/1157 remaining: 868)
  console.log('Completing Instagram Hashtag (868 remaining)...');
  ...
  for (let i = 290; i < igHashtagLines.length && igHashtagImported < 868; i++) {
    ...
    if (fields.length >= 11) {
      try {
        ...
        if (igHashtagImported % 100 === 0) {
          console.log(`  Instagram Hashtag: ${igHashtagImported}/868 imported`);
        }
      } catch (error) {
        // Skip invalid
      }
