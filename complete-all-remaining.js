...
async function completeAllRemaining() {
  ...
  // Complete remaining Instagram Hashtag (996/1157 remaining: 161)
  console.log('Completing Instagram Hashtag (161 remaining)...');
  const igHashtagContent = fs.readFileSync('public/Insta_new_nexthashtags_cleaned.xlsx_csv.csv', 'utf-8');
  const igHashtagLines = igHashtagContent.split('\n');
  let igHashtagImported = 0;
  
  for (let i = 997; i < igHashtagLines.length && igHashtagImported < 161; i++) {
  ...
  const ytOfficialContent = fs.readFileSync('public/dataset_youtube-channel-scraper_NextRetail-Official 1_cleaned.xlsx_csv.csv', 'utf-8');
